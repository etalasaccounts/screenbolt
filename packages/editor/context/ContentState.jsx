...
// `chooseReader` (video-source loading) and the download-delivery mechanism
// (save/export destination) are the two host-specific adapter points — see
// docs/specs/07-shared-editor.md. Each host injects its own implementation
// via props on <ContentState videoSource={...} saveDestination={...}>;
// apps/extension's defaults (recorderStorage + chrome.downloads) live in
// apps/extension/src/pages/Editor/editorAdapters.js and are unchanged from
...
const DEBUG_RECORDER =
  typeof window !== "undefined" ? !!window.SCREENBOLT_DEBUG_RECORDER : false;
...
const ContentState = (props) => {
  ...
  // Adapter injection points (see docs/specs/07-shared-editor.md): the host
  // app (apps/extension or apps/web) supplies its own implementation of
  // where the recording comes from and where the finished export goes.
  // `chooseReader` matches recorderStorage's ChunkReaderInterface contract
  // (open/readBlob/close); `saveDestination.deliver(url, filename, helpers)`
  // matches the extension's original chrome.downloads-based requestDownload
  // body 1:1 (see apps/extension/src/pages/Editor/editorAdapters.js).
  const videoSource = props.videoSource || {};
  const missingAdapter = (name) => async () => {
    throw new Error(
      `[packages/editor] no "${name}" adapter was provided to <ContentState>`,
    );
  };
  const chooseReader = videoSource.chooseReader || missingAdapter("videoSource.chooseReader");
  const saveDestination = props.saveDestination || {
    deliver: missingAdapter("saveDestination.deliver"),
  };
  ...
  useEffect(() => {
    if (!DEBUG_RECORDER && !isRecordingDebugEnabled()) return;
    window.__screenboltExportRecordingDebug = async () => {
  ...
  const waitForFinalizeReady = async (recordingId) => {
    ...
    return new Promise(async (resolve) => {
      ...
      const handleStatus = (status) => {
        ...
        if (DEBUG_POSTSTOP)
          console.debug("[Screenbolt][Sandbox] waitForFinalizeReady status", {
            status,
          });
  ...
  const buildBlobFromChunks = async () => {
    ...
    if (!readResult || readResult.chunkCount === 0) {
      if (DEBUG_POSTSTOP)
        console.warn(
          "[Screenbolt][Sandbox] buildBlobFromChunks: no parts found",
        );
    ...
    if (DEBUG_POSTSTOP)
      console.debug(
        "[Screenbolt][Sandbox] buildBlobFromChunks: reconstructed blob",
        {
          size: blob.size,
          type: blob.type,
  ...
  useEffect(() => {
    const loadInitialTitle = async () => {
      ...
      const fallbackTitle = `Screenbolt video - ${formattedDate}`;
  ...
  const reconstructVideo = async (withBlob) => {
    ...
    if (blob.type === "video/mp4" || isFastWebm) {
      if (DEBUG_RECORDER)
        console.log("[Screenbolt][Sandbox] reconstructVideo: fast path taken", {
          size: blob.size,
          type: blob.type,
          isFastWebm,
        });
    ...
    if (!recordingDuration || recordingDuration <= 0) {
      console.warn(
        "[Screenbolt][WebM] recordingDuration missing or 0, probing from blob",
      );
      try {
        ...
        console.warn("[Screenbolt][WebM] blob duration probe failed:", err);
      }
    }
    ...
    try {
      if (safeDuration > 0) {
        ...
        console.warn(
          "[Screenbolt][WebM] skipping duration fix: safeDuration=0, blob will have broken seek metadata",
        );
        if (
          contentStateRef.current.updateChrome ||
          contentStateRef.current.noffmpeg ||
      ...
      console.error(
        "[Screenbolt][WebM] duration fix failed, using unfixed blob:",
        error,
      );
      setContentState((prevState) => ({
        ...prevState,
    ...
    if (withBlob) {
      setTimeout(() => {
        ...
        if (s?.ready) return;
        console.warn(
          "[Screenbolt][WebM] reconstructVideo(blob) safety timeout: forcing ready with raw blob",
        );
        setContentState((prev) => {
          if (prev.ready) return prev;
          return {
  ...
  const makeVideoTab = async (sendResponse = null, message) => {
    ...
    if (DEBUG_POSTSTOP)
      console.debug("[Screenbolt][Sandbox] makeVideoTab invoked", {
    ...
    try {
      ...
      if (process.env.SCREENBOLT_DEV_MODE === "true") {
        console.log(
      ...
      if (lastRecordingBackendRef?.backend === "opfs") {
        ...
        for (let attempt = 1; attempt <= MAX_OPFS_READ_ATTEMPTS; attempt += 1) {
          try {
            ...
            console.warn(
              "[Screenbolt][Sandbox] OPFS direct read failed",
              { attempt, err },
            );
    ...
    const safetyCheck = () => {
      const s = contentStateRef.current;
      if (DEBUG_POSTSTOP)
        console.debug("[Screenbolt][Sandbox] makeVideoTab: safety-check", {
      ...
      if (complete && s?.rawBlob) {
        if (s?.webm) {
          if (DEBUG_RECORDER)
            console.log(
              "[Screenbolt][WebM] safety timeout: webm already set by fix, marking ready",
            );
          setContentState((prev) => ({
            ...prev,
            ready: true,
          ...
          console.warn(
            "[Screenbolt][WebM] safety timeout: duration fix did not complete in time, using unfixed rawBlob",
          );
          setContentState((prev) => ({
            ...prev,
            webm: prev.rawBlob,
    ...
    setTimeout(() => {
      if (!contentStateRef.current?.ready) {
        console.warn(
          "[Screenbolt][WebM] 60s safety timeout: force-marking ready",
        );
        safetyCheck();
      }
    }, 60000);
  ...
  const onChromeMessage = useCallback(
    (request, sender, sendResponse) => {
      ...
      if (DEBUG_POSTSTOP)
        console.debug("[Screenbolt][Sandbox] onChromeMessage", {
          type: message?.type,
      ...
      if (message.type === "chunk-count") {
        if (DEBUG_POSTSTOP)
          console.debug("[Screenbolt][Sandbox] received chunk-count", {
            count: message.count,
          });
        ...
        if (DEBUG_POSTSTOP)
          console.debug("[Screenbolt][Sandbox] received make-video-tab");
  ...
  useEffect(() => {
    ...
    const storageListener = (changes, areaName) => {
      ...
      try {
        ...
        if (changes[key]) {
          if (DEBUG_POSTSTOP)
            console.debug("[Screenbolt][Sandbox] storage fallback triggered", {
              key,
            });
          ...
          if (!window.indexedDB) {
            if (DEBUG_POSTSTOP)
              console.warn(
                "[Screenbolt][Sandbox] storage fallback: no indexedDB in this context, skipping",
              );
            return;
          }
          ...
          buildBlobFromChunks()
            .then((blob) => {
              if (!blob) {
                if (DEBUG_POSTSTOP)
                  console.warn(
                    "[Screenbolt][Sandbox] storage fallback: no blob built",
                  );
                return;
              }
              if (DEBUG_POSTSTOP)
                console.debug(
                  "[Screenbolt][Sandbox] storage fallback: blob built",
                  {
                    size: blob.size,
                  },
                );
            ...
            .catch((err) => {
              if (DEBUG_POSTSTOP)
                console.warn(
                  "[Screenbolt][Sandbox] storage fallback build error",
                  err,
                );
            });
        }
        ...
        if (DEBUG_POSTSTOP)
          console.warn("[Screenbolt][Sandbox] storageListener error", err);
      }
    };
  ...
  const onMessage = async (event) => {
    ...
    if (event.data.type === "updated-blob") {
      ...
      video.onloadedmetadata = async () => {
        if (process.env.SCREENBOLT_DEV_MODE === "true") {
          console.log("[Screenbolt][cut-debug] updated-blob received", {
      ...
      console.log("[Screenbolt][Editor] recording-complete sent from ffmpeg-load-error fallback");
      chrome.runtime.sendMessage({ type: "recording-complete" });
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      console.log("[Screenbolt][Editor] recording-complete sent from ffmpeg-load-timeout fallback");
      chrome.runtime.sendMessage({ type: "recording-complete" });
    }, 30000);
  ...
  const handleTrim = async (cut) => {
    ...
    if (process.env.SCREENBOLT_DEV_MODE === "true") {
      console.log("[Screenbolt][cut-debug] handleTrim dispatch", {
        cut,
        opId,
  ...
  // Save/export destination adapter point (see docs/specs/07-shared-editor.md):
  // the outer bookkeeping here (double-click guard, downloadInProgress flag,
  // filename sanitization, object-URL revocation) is host-agnostic and
  // unchanged; the actual "where does the file go / how do we know it's
  // done" mechanism is injected via `saveDestination.deliver`. apps/extension's
  // default implementation is the exact original chrome.downloads +
  // Brave-base64-fallback + timeout body, moved as-is into
  // apps/extension/src/pages/Editor/editorAdapters.js.
  const requestDownload = async (url, ext) => {
    ...
    if (contentStateRef.current?.downloadInProgress) {
      console.warn("[Screenbolt] download already in progress, ignoring");
      return;
    }
    setContentState((prev) => ({
      ...prev,
    ...
    const rawTitle = contentStateRef.current.title || "Screenbolt recording";
    ...
    try {
      await saveDestination.deliver(url, filename, {
        revoke,
        setContentState,
        contentStateRef,
        diagForward,
      ...
      // Idempotent (guarded by `revoked`) — a safety net so downloadInProgress
      // can never get stuck true if deliver() throws before calling revoke()
      // itself on some path we didn't anticipate.
      revoke();
    }
  };
  ...
  const remuxViaOffscreenOpfs = async (
    ...
    const devLog =
      process.env.SCREENBOLT_DEV_MODE === "true"
        ? (label, data) =>
  ...
  const produceStandardMp4 = async (blob) => {
    ...
    try {
      ...
      console.warn("[Screenbolt] offscreen remux failed, falling back", err);
    ...
    if (!remuxedBlob && !downloadCancelledRef.current) {
      try {
        ...
        console.warn(
          "[Screenbolt] buffer-target remux failed, falling back",
          err,
        );
  ...
  const download = async () => {
    ...
    try {
      ...
      console.warn("[Screenbolt] standard mp4 finalize failed", err);
    }
  ...
  const downloadWEBM = async () => {
    ...
    if (ENABLE_OFFSCREEN_WEBM) {
      ...
      try {
        ...
        console.warn(
          "[Screenbolt] offscreen webm convert failed, falling back",
          err,
        );
      }
      if (downloadCancelledRef.current) return;
      ...
      try {
        ...
        console.error("[Screenbolt] webm download failed", err);
      }
      setContentState((prevState) => ({
        ...prevState,
        downloadingWEBM: false,
    ...
    if (!completed) {
      ...
      try {
        ...
        console.error("[Screenbolt] webm fallback download failed", err);
      }
      setContentState((prevState) => ({
        ...prevState,
        downloadingWEBM: false,
  ...
  // Video-source adapter methods used directly by layout components (e.g.
  // RightPanel's OPFS-recovery download action) rather than by ContentState
  // itself — exposed on contentState so those components don't need their
  // own prop-drilling path back to the host-supplied adapter.
  contentState.downloadResolvedRecording = videoSource.downloadResolvedRecording;
  contentState.stageBlobToOpfs = videoSource.stageBlobToOpfs;
