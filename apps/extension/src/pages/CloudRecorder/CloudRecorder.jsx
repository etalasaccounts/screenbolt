...
// needing to set `window.SCREENBOLT_DEBUG_RECORDER = true` first. Prod
// builds keep the original flag.
const DEBUG_START_FLOW =
  typeof window !== "undefined"
    ? !!window.SCREENBOLT_DEBUG_RECORDER ||
...
const CloudRecorder = () => {
  ...
  const logStartFlow = (event, data = {}) => {
    ...
    console.warn("[Screenbolt][StartFlow]", payload);
  ...
  const assertCountdownBeforeFirstChunk = async (firstChunkAt) => {
    ...
    try {
      ...
      if (
        ...
        console.error("[Screenbolt][StartFlow] Chunk before countdown end", {
          firstChunkAt,
          countdownFinishedAt,
        });
      }
    } catch (err) {
      console.warn("[Screenbolt][StartFlow] Countdown assert failed", err);
    }
  };
  ...
  const resolveUploadTelemetryToken = async () => {
    ...
    try {
      ...
      if (screenboltToken) {
        uploadTelemetryTokenRef.current = screenboltToken;
        return screenboltToken;
      }
    } catch {}
  ...
  const sendUploadTelemetryNetwork = async (eventPayload) => {
    ...
    try {
      ...
      const headers = {
        ...
        "x-screenbolt-source": "extension",
      };
      if (eventPayload.extensionVersion) {
        headers["x-screenbolt-ext-version"] = String(
  ...
  const linkMediaToScene = async (projectId, sceneId, mediaId) => {
    ...
    await fetch(`${API_BASE}/media/${mediaId}/scene/`, {
      ...
      headers: {
        "Content-Type": "application/json",
        ...(screenboltToken ? { Authorization: `Bearer ${screenboltToken}` } : {}),
      },
  ...
  const stopTabKeepAlive = () => {
    try {
      ...
      if (globalThis.SCREENBOLT_VERBOSE_LOGS) {
        if (DEBUG_START_FLOW) {
  ...
  const startRecorderSession = async (meta = {}) => {
    ...
    try {
      ...
      if (result?.ok === false) {
        ...
        sendRecordingError(
          result?.error ||
            "Another Screenbolt recorder is already running. Please close it and try again.",
        );
        return false;
      }
    } catch (err) {
  ...
  const shouldSimulateFinalizeFailure = () => {
    try {
      ...
      return window.SCREENBOLT_SIMULATE_FINALIZE_FAILURE === true;
    } catch {
      return false;
    }
  };
    ...
    try {
      ...
      try {
        await chrome.downloads.download({
          url: objectUrl,
          filename: `Screenbolt-Finalize-Diagnostics-${new Date().toISOString()}.json`,
          saveAs: false,
        });
      } finally {
  ...
  const exportLocalRecovery = async (reason = "upload failed") => {
    try {
      ...
      try {
        await chrome.downloads.download({
          url: objectUrl,
          filename: `Screenbolt-Recovery-${new Date().toISOString()}-${reason}.webm`,
          saveAs: false,
        });
        chrome.runtime.sendMessage({
          type: "show-toast",
        ...
        if (chunkCount > 0) {
          ...
          if (blob) {
            ...
            try {
              await chrome.downloads.download({
                url: objectUrl,
                filename: `Screenbolt-Recovered-${ts}.webm`,
                saveAs: false,
              });
            } finally {
        ...
        if (cameraChunkCount > 0) {
          ...
          if (cameraBlob) {
            ...
            try {
              await chrome.downloads.download({
                url: cameraObjectUrl,
                filename: `Screenbolt-Recovered-Camera-${ts}.webm`,
                saveAs: false,
              });
            } finally {
              setTimeout(() => {
  ...
  const cleanupIfEmptyUploads = async (reason) => {
    ...
    console.warn(
      `[Screenbolt] empty upload cleanup reason=${reason} screenOffset=${screenOffset} cameraOffset=${cameraOffset}`,
    );
  ...
  const sendRecordingError = (why, cancel = false) => {
    console.error(
      `[Screenbolt][CloudRecorder] sendRecordingError why=${typeof why === "string" ? why : JSON.stringify(why)} cancel=${cancel}`,
    );
  ...
  const sendEditorReady = ({
    ...
    if (!shouldNotifyEditor || !projectId || !sceneId) {
      console.warn("[Screenbolt][CloudRecorder] Skipping editor-ready", {
        shouldNotifyEditor,
    ...
    console.info("[Screenbolt][CloudRecorder] Sending editor-ready", {
      projectId,
      sceneId,
      recordingToScene: Boolean(recordingToScene),
      multiMode: Boolean(multiMode),
    ...
    chrome.runtime.sendMessage({
      ...
      publicUrl: !recordingToScene
        ? `${process.env.SCREENBOLT_APP_BASE}/view/${projectId}/`
        : undefined,
      ...
      editorUrl:
        !recordingToScene && !multiMode
          ? instantMode.current
            ? `${process.env.SCREENBOLT_APP_BASE}/view/${projectId}?load=true`
            : `${process.env.SCREENBOLT_APP_BASE}/editor/${projectId}/edit?load=true`
          : undefined,
  ...
  const stopRecording = async (shouldFinalize = true, reason = "unknown") => {
    ...
    if (DEBUG_START_FLOW) {
      console.debug("[Screenbolt] stopRecording invoked", {
        reason,
        shouldFinalize,
      ...
      if (recordingToScene) {
        ...
        chrome.runtime.sendMessage({
          ...
          url: instantMode.current
            ? `${process.env.SCREENBOLT_APP_BASE}/view/${projectId}?load=true`
            : `${process.env.SCREENBOLT_APP_BASE}/editor/${projectId}/edit?load=true`,
          publicUrl: `${process.env.SCREENBOLT_APP_BASE}/view/${projectId}/`,
          instantMode: instantMode.current,
        });
      }
    }
        ...
        try {
          if (prewarmedStream) {
            ...
            if (typeof window !== "undefined") {
              window.__screenboltPrewarmedTabStream = null;
            }
  ...
  useEffect(() => {
    ...
    const sendReady = () => {
      window.parent.postMessage(
        { type: "screenbolt-region-capture-loaded" },
        "*",
      );
    };
