...
export default class BunnyTusUploader {
 ...
 async initialize(
  ...
  try {
   ...
   if (!this.videoId || !this.mediaId) {
    ...
    if (!authenticated) throw new Error("Not authenticated with Screenbolt");

    const { screenboltToken } = await chrome.storage.local.get([
     "screenboltToken",
    ]);

    this.userToken = screenboltToken;
 ...
 async refreshTusAuth() {
  ...
  const token = this.userToken || (await chrome.storage.local.get(["screenboltToken"]).then(r => r.screenboltToken));
