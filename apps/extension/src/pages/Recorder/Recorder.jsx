...
// Toggle: window.SCREENBOLT_DEBUG_RECORDER = true
const DEBUG_RECORDER =
  typeof window !== "undefined" ? !!window.SCREENBOLT_DEBUG_RECORDER : false;
const FORCE_MEDIARECORDER =
  typeof window !== "undefined"
    ? !!window.SCREENBOLT_FORCE_MEDIARECORDER
    : false;
const logPrefix = "[Screenbolt Recorder]";
...
function slLog(tag, extra = {}) {
  ...
  if (DEBUG_RECORDER) {
    ...
    console.log("[Screenbolt:SL]", tag, entry);
  }
...
const Recorder = () => {
  ...
  const startTabKeepAlive = () => {
    ...
    const inlineKA =
      typeof window !== "undefined" ? window.__SCREENBOLT_KEEPALIVE : null;
  ...
  async function saveChunk(e, i) {
    ...
    try {
      ...
      if (g.__screenboltForceChunkSaveQuotaError && !g.__screenboltForceChunkSaveQuotaError_fired) {
        g.__screenboltForceChunkSaveQuotaError_fired = true;
  ...
  function armStartGateTimeout() {
    ...
    startGateTimeout.current = setTimeout(() => {
      ...
      console.warn("[Screenbolt:startRec] stream never became ready", diagInfo);
  ...
  async function startRecording({ forceMediaRecorder = false } = {}) {
    ...
    if (!helperVideoStream.current) {
      ...
      console.warn("[Screenbolt:startRec] helperVideoStream is null", diagInfo);
    ...
    if (videoTracks.length === 0) {
      ...
      console.warn("[Screenbolt:startRec] helperVideoStream has 0 video tracks", diagInfo);
    ...
    }
    {
      ...
      if (vt.readyState === "ended") {
        console.warn("[Screenbolt:startRec] video track present but ended", diagInfo);
    ...
    const handleChunk = async (data, timestampMs) => {
      ...
      if (useWebCodecs.current) {
        ...
        try {
          ...
          const g = /** @type {any} */ (globalThis);
          if (
            g.__screenboltForceWebCodecsChunkSaveQuotaError &&
            !g.__screenboltForceWebCodecsChunkSaveQuotaError_fired
          ) {
            g.__screenboltForceWebCodecsChunkSaveQuotaError_fired = true;
