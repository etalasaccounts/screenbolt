...
import {
  ...
  getPairingState,
  clearDevicePairing,
} from "../pairing/pairingClient";
import { handleUploadToWeb } from "../upload/uploadToWeb";
import {
  getDiagnosticLog,
  getErrorSnapshot,
  getStorageFlags,
...
const API_BASE = process.env.SCREENBOLT_API_BASE_URL;
...
const markProjectPublic = async (projectId) => {
  ...
  try {
    const { screenboltToken } = await chrome.storage.local.get("screenboltToken");
    if (!screenboltToken) return;
    await fetch(`${API_BASE}/videos/${projectId}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${screenboltToken}`,
      },
...
// Off in prod by default; set globalThis.SCREENBOLT_DEBUG_RECORDER for support.
const DEBUG_FLOW =
  process.env.NODE_ENV !== "production" ||
  (typeof globalThis !== "undefined" && !!globalThis.SCREENBOLT_DEBUG_RECORDER);
...
const scheduleLocalPlaybackAlarm = async (offer) => {
  ...
  try {
    ...
    console.warn("[Screenbolt][BG] Failed to schedule local playback alarm", err);
  }
};
...
const clearStoredLocalPlaybackOffer = async ({
  ...
  if (clearChunks) {
    ...
    await targetStore.clear().catch((err) => {
      console.warn(
        "[Screenbolt][BG] Failed to clear screen chunks while clearing local playback offer",
        err,
      );
    });
    if (
  ...
  if (existing?.offerId) {
    console.info("[Screenbolt][BG] Cleared local screen playback offer", {
      reason,
...
const logStopRecordingTabEvent = (message, sender) => {
  try {
    ...
    console.warn("[Screenbolt][BG] stop-recording-tab received", {
      reason,
      senderTabId,
      senderUrl,
    });
    ...
    console.warn("[Screenbolt][BG] stop-recording-tab logging failed", err);
  }
};
...
export const handleFinishMultiRecording = async () => {
  try {
    ...
    if (!recordingToScene) {
      ...
      const url = `${process.env.SCREENBOLT_APP_BASE}/editor/${multiProjectId}/edit?share=true`;
      const publicUrl = `${process.env.SCREENBOLT_APP_BASE}/view/${multiProjectId}/`;
      ...
      if (messageTab) {
        ...
        await sendMessageTab(messageTab, {
          ...
          console.warn(
            "[Screenbolt][BG] Failed to send update-project-ready (finish-multi-recording)",
            err,
          ),
        );
      } else {
        console.warn(
          "[Screenbolt][BG] No tab available for update-project-ready (finish-multi-recording)",
  ...
  recordingTabListener = (closedTabId) => {
    if (closedTabId === ownerTabId) {
      ...
      Promise.resolve(
        ...
        console.error(
          "[Screenbolt][BG] handleStopRecordingTab failed in tab-removed",
          err,
        );
      });
...
const resolveActiveSessionConflict = async (incomingSession) => {
  ...
  if (alive) {
    console.warn("[Screenbolt][BG] session_conflict_rejected", {
  ...
  console.warn("[Screenbolt][BG] session_conflict_stale_recovered", {
    incomingId: incomingSession.id,
  });
  return { allow: true, staleRecovered: true };
};
...
export const setupHandlers = () => {
  ...
  registerMessage("forward-create-scene", async (message) => {
    ...
    try {
      const { screenboltToken } = await chrome.storage.local.get([
        "screenboltToken",
      ]);
      const headers = { "Content-Type": "application/json" };
      if (screenboltToken) headers.Authorization = `Bearer ${screenboltToken}`;
    ...
    const targetUrl = `${process.env.SCREENBOLT_APP_BASE}/editor/${projectId}/edit?load=true`;
    const resolved = await resolveEditorTabForTarget({
      targetUrl,
  ...
  registerMessage("offscreen-diag", async (message) => {
    ...
    console.warn("[Screenbolt][OffscreenDiag]", message.source, payloadStr);
    return { ok: true };
  });
  ...
  registerMessage("stop-recording-tab", (message, sender, sendResponse) => {
    ...
    if (
      ...
      if (DEBUG_POSTSTOP) {
        console.warn(
          "[Screenbolt][BG] Suppressed duplicate stop-recording-tab message",
          {
  ...
  // chrome.runtime.id is this build's actual webstore listing id, so this
  // opens the extension's own review page regardless of which store entry
  // it's published under.
  registerMessage("review-webstore", () =>
    createTab(
  ...
  registerMessage("pricing", (message) => {
    const source =
      typeof message?.source === "string" && message.source
        ...
        : "extension";
    return createTab(
      `https://screenbolt.app/?ref=${encodeURIComponent(source)}`,
      true,
    );
  });
  registerMessage("open-processing-info", () =>
    createTab(
      "https://help.screenbolt.app/editing-and-exporting/dJRFpGq56JFKC7k8zEvsqb/why-is-there-a-5-minute-limit-for-editing/ddy4e4TpbnrFJ8VoRT37tQ",
      true,
    ),
  );
  registerMessage("upgrade-info", () =>
    createTab(
      "https://help.screenbolt.app/getting-started/77KizPC8MHVGfpKpqdux9D/what-are-the-technical-requirements-for-using-screenbolt/6kdB6qru6naVD8ZLFvX3m9",
      true,
    ),
  );
  registerMessage("trim-info", () =>
    createTab(
      "https://help.screenbolt.app/editing-and-exporting/dJRFpGq56JFKC7k8zEvsqb/how-to-cut-trim-or-mute-parts-of-your-video/svNbM7YHYY717MuSWXrKXH",
      true,
    ),
  );
  ...
  registerMessage("open-help", () =>
    createTab("https://help.screenbolt.app/", true),
  );
  registerMessage("memory-limit-help", () =>
    createTab(
      "https://help.screenbolt.app/troubleshooting/9Jy5RGjNrBB42hqUdREQ7W/what-does-%E2%80%9Cmemory-limit-reached%E2%80%9D-mean-when-recording/8WkwHbt3puuXunYqQnyPcb",
      true,
    ),
  );
  registerMessage("open-home", () =>
    createTab("https://screenbolt.app/", true),
  );
  ...
  registerMessage("submit-diagnostic-report", async (message) => {
    try {
      const appBase = process.env.SCREENBOLT_APP_BASE;
      if (!appBase) return;
      const { startFlowTrace, screenboltToken, projectId, recorderSession } =
        await chrome.storage.local.get([
          "startFlowTrace",
          "screenboltToken",
          "projectId",
      ...
      if (!startFlowTrace || !screenboltToken) return;
  ...
  // Device pairing + upload to apps/web (docs/specs/04-integration.md).
  // Not gated behind CLOUD_FEATURES_ENABLED -- see webApp/config.js's
  // WEB_UPLOAD_ENABLED for why this is a separate flag.
  registerMessage("start-pairing", async () => await startDevicePairing());
  registerMessage("get-pairing-state", async () => await getPairingState());
  registerMessage("clear-pairing", async () => await clearDevicePairing());
  registerMessage(
    "upload-to-web",
    async (message) => await handleUploadToWeb(message, false),
  );
  registerMessage(
    "upload-to-web-fallback",
    async (message) => await handleUploadToWeb(message, true),
  );
  ...
  registerMessage("handle-login", async () => {
    ...
    const loginTab = await chrome.tabs.create({
      url: `${process.env.SCREENBOLT_APP_BASE}/login?extension=true`,
      active: true,
    });
  ...
  registerMessage("handle-logout", async (message, sender, sendResponse) => {
    ...
    // keep screenboltToken during active recording for bunnyTusUploader.refreshTusAuth;
    ...
    if (!recordingBusy) {
      removeKeys.push("screenboltToken");
    }
  ...
  registerMessage("prepare-open-editor", async (message) => {
    ...
    console.info("[Screenbolt][BG] prepare-open-editor", {
      projectId: expectedProjectId,
      targetUrl,
      instantMode: Boolean(message.instantMode),
    ...
    console.info("[Screenbolt][BG] prepare-open-editor resolved", {
  ...
  registerMessage("prepare-editor-existing", async (message) => {
    ...
    if (messageTab) {
      ...
      await sendMessageTab(messageTab, {
        ...
        console.warn(
          "[Screenbolt][BG] Failed to send update-project-loading",
          err,
        ),
      );
    } else {
  ...
  registerMessage("editor-ready", async (message) => {
    ...
    console.info("[Screenbolt][BG] editor-ready received", {
    ...
    if (messageTab) {
      await sendMessageTab(messageTab, {
        ...
        console.warn("[Screenbolt][BG] Failed to send update-project-ready", err),
      );
    } else {
  ...
  registerMessage("cloud-local-playback-register", async (message) => {
    ...
    console.info("[Screenbolt][BG] Registered local screen playback offer", {
      offerId: normalizedOffer.offerId,
      projectId: normalizedOffer.projectId,
      sceneId: normalizedOffer.sceneId,
      chunkCount: normalizedOffer.chunkCount,
  ...
  registerMessage("cloud-local-playback-mark-used", async (message) => {
    ...
    console.info("[Screenbolt][BG] Local screen playback offer marked used", {
      offerId: updated.offerId,
      projectId: updated.projectId,
      sceneId: updated.sceneId,
    });
  ...
  registerMessage("cloud-local-playback-mark-fallback", async (message) => {
    ...
    console.info("[Screenbolt][BG] Local screen playback offer fallback", {
      offerId: updated.offerId,
      reason: updated.fallbackReason,
    });
  ...
  registerMessage("handle-reactivate", async () => {
    ...
    chrome.tabs.create({
      url: `${process.env.SCREENBOLT_APP_BASE}/reactivate`,
      active: true,
    });
  });
  registerMessage("handle-upgrade", async () => {
    ...
    chrome.tabs.create({
      url: `${process.env.SCREENBOLT_APP_BASE}/upgrade`,
      active: true,
    });
  });
  registerMessage("open-account-settings", async () => {
    ...
    const url = `${process.env.SCREENBOLT_APP_BASE}/?settings=open`;
    createTab(url, true);
  });
  ...
  registerMessage("activate-recorder-tab", async (message, sender) => {
    ...
    if (tabId) {
      try {
        ...
        console.warn("[Screenbolt] activate-recorder-tab failed:", String(err));
      }
    }
  });
