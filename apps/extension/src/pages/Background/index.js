...
const clearStaleLocks = async () => {
  try {
    ...
    if (sendingChunks) {
      ...
      console.warn("[Screenbolt][BG] Stale lock found on startup: sendingChunks, clearing");
    }
    if (postStopEditorOpening) {
      stale.postStopEditorOpening = false;
      console.warn("[Screenbolt][BG] Stale lock found on startup: postStopEditorOpening, clearing");
    }
    if (postStopEditorOpened) {
      stale.postStopEditorOpened = false;
      console.warn("[Screenbolt][BG] Stale lock found on startup: postStopEditorOpened, clearing");
    }
    ...
    if (recording || pendingRecording || restarting) {
      ...
      if (!recorderAlive && startIsFresh) {
        console.warn(
          "[Screenbolt][BG] Stale-looking start on startup but within start grace, keeping",
      ...
      if (!recorderAlive && !startIsFresh) {
        ...
        console.warn(
          "[Screenbolt][BG] Stale recording state on startup (no live recorder tab or offscreen doc) - clearing",
    ...
    if (multiMode && !recording) {
      ...
      console.warn("[Screenbolt][BG] Stale multi-mode state found on startup, clearing");
    }

    if (region && !recording) {
      stale.region = false;
      console.warn("[Screenbolt][BG] Stale region state found on startup, clearing");
    }

    if (Object.keys(stale).length > 0) {
      await chrome.storage.local.set(stale);
      console.info(
        "[Screenbolt][BG] Startup stale locks cleared:",
    ...
    try {
      ...
      if (
        ...
        if (stayLoggedOut === true) {
          ...
          console.info(
            "[Screenbolt][BG] Drained deferred logout token-clear on startup",
          );
        } else {
    ...
    try {
      ...
      console.warn("[Screenbolt][BG] icon reconciliation failed:", err);
    }
  } catch (err) {
    console.error("[Screenbolt][BG] Failed to clear stale startup locks:", err);
  }
};
...
const recoverInFlightRecording = async () => {
  try {
    ...
    try {
      ...
      console.warn("[Screenbolt][BG] redeliver loaded failed:", err);
    }
    ...
    handleGetStreamingData().catch((err) => {
      console.warn("[Screenbolt][BG] redeliver streaming-data failed:", err);
    });
  } catch (err) {
    console.warn("[Screenbolt][BG] recoverInFlightRecording threw:", err);
  }
};
...
const cleanupOrphanOpfsSessions = async () => {
  try {
    ...
    console.info(
      "[Screenbolt][BG] Reaped orphan OPFS cloud-chunks sessions:",
      orphans.length,
    );
  } catch (err) {
    console.warn(
      "[Screenbolt][BG] cleanupOrphanOpfsSessions failed:",
      err,
    );
  }
};
...
const CURRENT_MIGRATION_VERSION = "4.4.0-dropLegacyStorageKeys";
const runUpgradeMigrations = async () => {
  try {
    const { screenboltMigratedForVersion } = await chrome.storage.local.get([
      "screenboltMigratedForVersion",
    ]);
    if (screenboltMigratedForVersion === CURRENT_MIGRATION_VERSION) return;
    ...
    await chrome.storage.local.set({
      screenboltMigratedForVersion: CURRENT_MIGRATION_VERSION,
    });
    console.info(
      "[Screenbolt][BG] Cleared stale 4.3.7 sticky-disable flags on upgrade",
    );
  } catch (err) {
    console.error("[Screenbolt][BG] Upgrade migration failed:", err);
  }
};
