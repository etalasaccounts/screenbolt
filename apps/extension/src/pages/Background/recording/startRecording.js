...
const _startRecordingInner = async (caller) => {
  ...
  try {
    ...
    if (
      ...
      console.warn(
        "[Screenbolt][BG] startRecording aborted: prior recording stopped",
        sinceStopMs,
        "ms ago, caller:",
        caller,
      );
  ...
  void (async () => {
    try {
      const tokenSnap = await chrome.storage.local.get(["screenboltToken"]);
      if (!tokenSnap?.screenboltToken) return;
      ...
      const userObj = extras?.screenboltUser || null;
...
export const startAfterCountdown = (caller = "startAfterCountdown") => {
  chrome.storage.local.get(["recordingTab", "offscreen"], (result) => {
    ...
    if (recordingTab === null && !offscreen) {
      console.warn(
        "[Screenbolt] startAfterCountdown: no recordingTab/offscreen available, starting with fallback routing"
      );
    }
