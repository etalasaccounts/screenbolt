...
export const handleChunks = async (chunks, override = false, target = null) => {
  ...
  if (DEBUG_POSTSTOP)
    console.debug("[Screenbolt][BG] handleChunks called", {
  ...
  try {
    if (!Array.isArray(chunks) || chunks.length === 0) {
      if (DEBUG_POSTSTOP)
        console.debug("[Screenbolt][BG] no chunks to send; deferring delivery");
      return;
    }
    ...
    if (DEBUG_POSTSTOP)
      console.debug("[Screenbolt][BG] sending chunk-count", {
        count: chunks.length,
        targetTab,
        targetFrame,
        override,
    ...
    try {
      ...
      if (DEBUG_POSTSTOP)
        console.warn("[Screenbolt][BG] chunk-count message failed", err);
    }
    ...
    if (bannerSupport) {
      try {
        ...
        if (DEBUG_POSTSTOP)
          console.warn("[Screenbolt][BG] banner-support message failed", err);
      }
    }
    ...
    if (DEBUG_POSTSTOP)
      console.debug(
        "[Screenbolt][BG] instructing sandbox to make video tab",
    ...
    try {
      ...
      if (DEBUG_POSTSTOP)
        console.warn("[Screenbolt][BG] make-video-tab message failed", err);
    }
  } finally {
