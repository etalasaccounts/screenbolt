...
export const restoreRecording = async () => {
    ...
    chrome.tabs.create({ url: editorUrl, active: true }, async (tab) => {
      ...
      await new Promise((resolve) => {
        ...
        const safetyId = setTimeout(() => {
          ...
          console.warn(
            "[Screenbolt][BG] restore-recording (opfs) tab load timed out",
            tab.id,
          );
  ...
  try {
    ...
    console.warn("[Screenbolt][BG] restoreRecording chunksStore.iterate failed", err);
  }
  if (chunks.length === 0) {
    console.warn("[Screenbolt][BG] restoreRecording: no OPFS files and no IDB chunks");
  ...
  chrome.tabs.create({ url: editorUrl, active: true }, async (tab) => {
    ...
    await new Promise((resolve) => {
      ...
      const safetyId = setTimeout(() => {
        ...
        chrome.tabs.onUpdated.removeListener(listener);
        console.warn(
          "[Screenbolt][BG] restore-recording (idb) tab load timed out",
          tab.id,
        );
        notifyRestoreTimeout(triggerTabId).catch(() => {});
        setTimeout(() => {
