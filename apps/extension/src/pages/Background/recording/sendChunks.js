...
export const sendChunks = async (override = false, target = null) => {
  ...
  try {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      console.debug("[Screenbolt][BG] sendChunks: chunkCount check", {
        attempt,
        chunkCount,
      });
    ...
    if (chunkCount === 0) {
      console.warn(
        "[Screenbolt][BG] sendChunks: no chunks available after waiting",
      );
      try {
    ...
    console.debug("[Screenbolt][BG] sendChunks: collected chunks", {
      count: chunks.length,
    });
