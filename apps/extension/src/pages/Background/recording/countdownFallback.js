...
const scheduleFallback = (delayMs = FALLBACK_AFTER_COUNTDOWN_STARTED_MS) => {
  ...
  fallbackTimerId = setTimeout(async () => {
    ...
    try {
      ...
      console.info(
        "[Screenbolt][BG] countdown-finished missing after delay, auto-dispatching start",
      );
      try {
        startAfterCountdown("countdownFallback");
      } catch (err) {
        console.error(
          "[Screenbolt][BG] countdownFallback startAfterCountdown failed",
          err,
        );
      }
    } catch (err) {
      console.error("[Screenbolt][BG] countdownFallback storage read failed", err);
    }
  }, delayMs);
};
...
export const recoverPendingCountdownOnStartup = async () => {
  try {
    ...
    console.info(
      "[Screenbolt][BG] recoverPendingCountdownOnStartup: SW restarted past countdown deadline, dispatching",
      { elapsedMs: elapsed },
    );
    try {
      ...
      console.error(
        "[Screenbolt][BG] recoverPendingCountdownOnStartup startAfterCountdown failed",
        err,
      );
    }
  } catch (err) {
    console.error(
      "[Screenbolt][BG] recoverPendingCountdownOnStartup storage read failed",
      err,
    );
  }
};
