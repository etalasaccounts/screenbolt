...
const openRecorderTab = async (
  ...
  if (willUseOffscreen) {
    ...
    try {
      ...
      console.error(
        "[Screenbolt][BG] openRecorderTab: offscreen create failed",
        err,
      );
      chrome.runtime
        .sendMessage({
    ...
    (async () => {
      ...
      console.warn(
        "[Screenbolt][BG] offscreen recorder loaded-send exhausted retries",
      );
    })();
    setTimeout(() => {
  ...
  if (!tab || typeof tab.id !== "number") {
    console.error(
      "[Screenbolt][BG] openRecorderTab: tabs.create returned no usable tab",
      tab,
    );
    chrome.runtime
      .sendMessage({
  ...
  for (let attempt = 0; attempt < backoffsMs.length; attempt += 1) {
    try {
      ...
      if (attempt === backoffsMs.length - 1) {
        console.warn(
          "[Screenbolt] autoDiscardable failed for recorder tab",
          tab.id,
          String(err),
        );
        chrome.storage.local.set({
  ...
  chrome.tabs.onUpdated.addListener(function _(tabId, changeInfo) {
    if (tabId === tab.id && changeInfo.status === "complete") {
      ...
      console.warn(
        "[Screenbolt][openRecorderTab] loadedMsg",
        JSON.stringify({
          isRegion,
          camera,
...
export const startRecorderSession = async (request, tabId = null) => {
  ...
  console.log("[Screenbolt][startRecorderSession] entered", { request, tabId });
