...
const acquirePostStopEditorLock = async (recordingId = null) => {
  ...
  try {
    ...
    console.warn(
      "[Screenbolt][BG] acquirePostStopEditorLock storage mirror timed out; in-memory lock holds",
    );
...
export const handleStopRecordingTab = async (request) => {
  ...
  if (!isSubscribed) {
    ...
    if (!lockAcquired) {
      console.warn(
        "[Screenbolt][BG] Duplicate stop-recording-tab suppressed (editor opening)",
      );
      ...
      chrome.tabs.create(
        ...
        (tab) => {
          ...
          const safetyTimer = setTimeout(async () => {
            ...
            settled = true;
            console.warn("[Screenbolt][BG] Editor tab load timed out; releasing lock");
              ...
              (async () => {
                ...
                if (storage.lastRecordingBackendRef?.backend === "opfs") {
                  ...
                  sendMessageTab(tab.id, { type: "make-video-tab" }).catch(
                    (err) => {
                      console.warn(
                        "[Screenbolt][BG] make-video-tab direct send failed",
                        err,
                      );
                    },
                  );
                ...
                if (!sent) {
                  console.warn(
                    "[Screenbolt][BG] editor opened but chunks are still unavailable",
                  );
                  ...
                  try {
                    ...
                    console.error(
                      "[Screenbolt][BG] failed to write editorRecordingError",
                      writeErr,
                    );
                  }
                }
