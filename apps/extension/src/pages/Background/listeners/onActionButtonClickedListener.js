...
const handleTabMessaging = async (tab) => {
  ...
  if (offscreen) {
    try {
      ...
      console.error(
        "[Screenbolt][ActionClick] offscreen stop-recording send failed:",
        err,
      );
      return;
    }
  ...
  try {
    ...
    console.error("[Screenbolt][ActionClick] handleTabMessaging failed, trying direct recorder stop:", error);
    try {
      await sendMessageRecord({ type: "stop-recording-tab" });
    } catch (recorderErr) {
      console.error("[Screenbolt][ActionClick] direct recorder stop also failed:", recorderErr);
    }
  }
};

const openPlaygroundOrPopup = async (tab) => {
  // Inherited from the original extension's editor-tab-reuse check. apps/web
  // never serves a standalone /editor/:id tab (its editor only renders
  // inside an iframe -- see app/record/editor-frame/page.tsx), so this
  // pattern has no current match and the branch below is effectively dead.
  const editorUrlPattern =
    /https:\/\/app\.screenbolt\.app\/editor\/([^\/]+)(\/edit)?\/?/;
  ...
  if (!isForbidden || isPlaygroundOrSetup) {
    sendMessageTab(tab.id, { type: "toggle-popup" })
      .then(() => console.log("[Screenbolt][ActionClick] toggle-popup delivered to tab", tab.id))
      .catch((err) => console.error("[Screenbolt][ActionClick] toggle-popup FAILED to tab", tab.id, String(err).slice(0, 120)));
    chrome.storage.local.set({ activeTab: tab.id });
  } else {
...
export const onActionButtonClickedListener = () => {
  chrome.action.onClicked.addListener(async (tab) => {
    ...
    try {
      ...
      console.log("[Screenbolt][ActionClick] storage:", snap, "tab:", tab.id);
      ...
      if (isRecordingActive) {
        ...
        if (sessionRecording) {
          ...
          if (sessionOwnerTabId && (await doesTabExist(sessionOwnerTabId))) {
            ...
            console.warn(
              "[Screenbolt][ActionClick] recorderSession stale (owner tab",
              sessionOwnerTabId,
              "is dead) — clearing",
            );
        ...
        if (recordingTab && !hasActiveRecorder && !sessionTerminal) {
          if (await doesTabExist(recordingTab)) {
            ...
            console.warn("[Screenbolt][ActionClick] recordingTab", recordingTab, "is dead — clearing");
        ...
        if (offscreen && !hasActiveRecorder && !sessionTerminal) {
          if (await isOffscreenAlive()) {
            ...
            console.warn("[Screenbolt][ActionClick] offscreen flag stale (no document) — clearing");
        ...
        if (!hasActiveRecorder) {
          console.warn(
            "[Screenbolt][ActionClick] branch: stale-reset-then-popup.",
          ...
          console.log("[Screenbolt][ActionClick] branch: handle-tab-messaging (active recorder)");
          await handleTabMessaging(tab);
        }
      } else {
        console.log("[Screenbolt][ActionClick] branch: normal-popup");
        await chrome.storage.local.set({
          recordingToScene: false,
          projectId: null,
          activeSceneId: null,
