...
export const handleAlarm = async (alarm) => {
  if (alarm.name === RECORDER_KEEPALIVE_ALARM) {
    ...
    if (stallLevel >= 1) {
      ...
      try {
        ...
        console.warn("[Screenbolt][BG] stall-unrecoverable handler failed", err);
      }
      return;
    }
    return;
    ...
    if (recording) {
      ...
      if (vt && vt.readyState === "live" && vt.muted === true) {
        ...
        try {
          ...
          console.warn("[Screenbolt][BG] watchdog sticky-disable failed", err);
        }
        ...
        try {
          ...
          console.warn("[Screenbolt][BG] first-chunk watchdog handler failed", err);
        }
      }
    }
    await chrome.alarms.clear(FIRST_CHUNK_WATCHDOG_ALARM);
  ...
  if (alarm.name === PAIRING_POLL_ALARM) {
    try {
      await runPairingAlarmBackstop();
    } catch (err) {
      console.warn("[Screenbolt][BG] pairing alarm backstop failed", err);
    }
    return;
  }
  ...
  if (alarm.name === CLOUD_LOCAL_PLAYBACK_ALARM) {
    ...
    await expiredStore.clear().catch((err) => {
      console.warn(
        "[Screenbolt][BG] Failed to clear chunk store for local playback expiry",
        err,
      );
    });
    if (
