...
const PopupContainer = (props) => {
  ...
  const [URL, setURL] = useState("https://help.screenbolt.app/");
  const isCloudBuild = process.env.SCREENBOLT_ENABLE_CLOUD_FEATURES === "true";
  ...
  useEffect(() => {
    const buildURL = async () => {
      let baseURL = "https://help.screenbolt.app/";

      if (contentState?.isLoggedIn && contentState?.screenboltUser) {
        const { name, email } = contentState.screenboltUser;
  ...
  const onValueChange = (tab) => {
    ...
    if (contentState.isLoggedIn && contentState.isSubscribed === false) {
      ...
      const avatar = contentState?.screenboltUser?.avatar;
