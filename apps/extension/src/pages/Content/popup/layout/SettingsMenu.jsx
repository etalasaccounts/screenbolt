...
const DEV_MODE = process.env.SCREENBOLT_DEV_MODE === "true";
...
const SettingsMenu = (props) => {
  ...
  // Device-pairing status with apps/web (docs/specs/04-integration.md),
  // refreshed whenever this menu opens.
  ...
  return (
    <DropdownMenu.Root
      ...
      onOpenChange={(open) => {
        ...
        chrome.runtime
          .sendMessage({ type: "get-pairing-state" })
          .then((response) => {
            setPairingInfo(response || { status: "idle", hasToken: false });
          })
          .catch(() => {});
      ...
      >
        <DropdownMenu.Content className="DropdownMenuContent" sideOffset={5}>
            ...
            >
              ...
              <DropdownMenu.Portal>
                <DropdownMenu.SubContent
                  className="ScreenboltDropdownMenuContent"
                  sideOffset={0}
                  alignOffset={-3}
                >
                  <TooltipWrap
                  ...
                  >
                    <DropdownMenu.Item
                      className="ScreenboltDropdownMenuItem"
                      onClick={(e) => {
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        4k
                        <DropdownMenu.ItemIndicator className="ScreenboltItemIndicator">
          ...
          <DropdownMenu.Item
            ...
            onSelect={(e) => {
              e.preventDefault();
              if (pairingInfo.hasToken) {
                chrome.runtime.sendMessage({ type: "clear-pairing" });
                setPairingInfo({ status: "idle", hasToken: false });
                contentState.openToast?.(
                  chrome.i18n.getMessage("screenboltDisconnectedToast") ||
                    "Disconnected from Screenbolt",
                  () => {},
                  2000
                );
              } else {
                chrome.runtime.sendMessage({ type: "start-pairing" });
                setPairingInfo({ status: "pending", hasToken: false });
                contentState.openToast?.(
                  chrome.i18n.getMessage("screenboltPairingStartedToast") ||
                    "Approve this device in the new tab",
                  () => {},
                  3000
                );
              }
          ...
          >
            {pairingInfo.hasToken
              ? chrome.i18n.getMessage("disconnectScreenboltOption") ||
                "Disconnect Screenbolt"
              : pairingInfo.status === "pending"
              ? chrome.i18n.getMessage("connectScreenboltPendingOption") ||
          ...
          {contentState.isLoggedIn && !CLOUD_FEATURES_ENABLED && (
            <DropdownMenu.Item
              ...
              onSelect={(e) => {
                ...
                chrome.runtime.sendMessage({
                  type: "open-support",
                  name: contentState.screenboltUser?.name || "",
                  email: contentState.screenboltUser?.email || "",
                });
              }}
            >
