...
export const setupHandlers = () => {
 if (window.__screenboltSetupHandlersRan) return;
 window.__screenboltSetupHandlersRan = true;
 ...
 const BRIDGE_READY = "screenbolt-local-playback-bridge-ready";
 const BRIDGE_REQUEST = "screenbolt-local-playback-bridge-request";
 const BRIDGE_RESULT = "screenbolt-local-playback-bridge-result";
 const BRIDGE_ORIGIN = (() => {
  try {
 ...
 const postProjectHandoff = (payload) => {
  ...
  if (!targetOrigin) {
   console.warn(
    "[Screenbolt][Content] Ignoring project handoff on untrusted origin",
    {
 ...
 const revokeActiveLocalPlaybackSource = (reason = "unknown") => {
  if (activeLocalPlaybackSource?.url) {
   ...
   console.info("[Screenbolt][Content] Revoked local screen playback URL", {
    reason,
 ...
 const fetchLocalPlaybackSourceFromExtension = async ({
  ...
  try {
   ...
   console.warn(
    "[Screenbolt][Content] Local playback bridge unavailable, falling back to runtime copy",
 ...
 const onWindowProjectMessage = (event) => {
  ...
  if (data?.type !== "screenbolt-local-playback-request") return;
  ...
  if (
   ...
   postProjectHandoff({
    source: "screenbolt-local-playback-response",
    requestId,
  ...
  void ensureLocalPlaybackReady({
   ...
   .then((readySource) => {
    console.info("[Screenbolt][Content] Local screen playback used", {
     projectId: requestedProjectId,
   ...
   .catch((err) => {
    ...
    console.warn(
     "[Screenbolt][Content] Local screen playback fallback",
     {
      projectId: requestedProjectId,
      sceneId: requestedSceneId || latestLocalPlaybackSceneId || null,
      offerId: offer.offerId,
 ...
 if (!window.__screenboltHandlersInitialized) {
  messageRouter();
  window.__screenboltHandlersInitialized = true;
 }
 ...
 registerMessage("start-stream", () => {
  ...
  if (
   ...
   console.warn("[Screenbolt][Content] start-stream BLOCKED by guard state:", {
 ...
 registerMessage("fast-recorder-hard-fail", async () => {
  ...
  const downloadBundle = async () => {
   ...
   const filename = "screenbolt-troubleshooting.zip";
   ...
   if (!resp?.ok || typeof resp.base64 !== "string") {
    console.warn("[Screenbolt] troubleshooting zip failed:", resp?.error);
    return;
   }
 ...
 registerMessage("reopen-popup-multi", async (message) => {
  ...
  setTimer(0);
  try {
   const elements = document.querySelectorAll(".screenbolt-blur");
   elements.forEach((el) => el.classList.remove("screenbolt-blur"));
  } catch {}
 ...
 registerMessage("update-project-ready", (message, sender) => {
  ...
  if (!projectId) {
   console.warn(
    "[Screenbolt][Content] Ignoring update-project-ready without projectId",
   );
   return;
  }
  ...
  if (posted) {
   window.__screenboltLastProjectReady = {
    projectId,
  ...
  if (posted && capturedOffer?.offerId) {
   ...
   console.info("[Screenbolt][Content] Local screen playback offered", {
    projectId,
   ...
   void ensureLocalPlaybackReady({
    ...
    .then((readySource) => {
     console.info("[Screenbolt][Content] Local screen playback ready", {
      projectId,
      sceneId: capturedSceneId,
      offerId: capturedOffer.offerId,
      bytes: readySource?.size || 0,
    ...
    .catch((err) => {
     const reason = err?.message || "local-playback-build-failed";
     console.warn("[Screenbolt][Content] Local screen playback fallback", {
      projectId,
      sceneId: capturedSceneId,
      offerId: capturedOffer.offerId,
      reason,
