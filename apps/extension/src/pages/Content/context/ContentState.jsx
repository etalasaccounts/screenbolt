...
const ContentState = (props) => {
  ...
  const [URL] = useState(
    "https://help.screenbolt.app/getting-started/77KizPC8MHVGfpKpqdux9D/why-does-screenbolt-ask-for-permissions/9AAE8zJ6iiUtCAtjn4SUT1",
  );
  const [URL2] = useState(
    "https://help.screenbolt.app/troubleshooting/9Jy5RGjNrBB42hqUdREQ7W/how-to-grant-screenbolt-permission-to-record-your-camera-and-microphone/x6U69TnrbMjy5CQ96Er2E9",
  );
  ...
  const stopRecording = useCallback(() => {
    ...
    elements.forEach((element) => {
      element.classList.remove("screenbolt-blur");
    });
    ...
    if (
      ...
      if (success && canUpload === false) {
        contentStateRef.current.openModal(
          ...
          () => {
            window.open(process.env.SCREENBOLT_APP_BASE, "_blank");
          },
          () => {},
        );
      } else if (!success) {
  ...
  useEffect(() => {
    const handleMessage = (event) => {
      if (event.data.type === "screenbolt-permissions") {
        handleDevicePermissions(event.data);
      } else if (event.data.type === "screenbolt-permissions-loaded") {
        setContentState((prevContentState) => ({
          ...prevContentState,
          permissionsLoaded: true,
        }));
      } else if (event.data.type === "screenbolt-site-policy") {
  ...
  useEffect(() => {
    const onChanged = (changes, area) => {
      ...
      if (
        changes.screenboltToken ||
        changes.screenboltUser ||
  ...
  useEffect(() => {
    if (window.__screenboltSetupHandlersInitialized) return;
    window.__screenboltSetupHandlersInitialized = true;
    setupHandlers();
  }, []);
  ...
  useEffect(() => {
    ...
    elements.forEach((element) => {
      element.classList.add("screenbolt-scrollbar");
    });
        ...
        if (mutation.type === "childList") {
          ...
          addedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              node.classList.add("screenbolt-scrollbar");
            }
          });

          removedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              node.classList.remove("screenbolt-scrollbar");
            }
          });
        }
      }
