...
const ZoomContainer = () => {
  ...
  const getCanvasWrapper = () => {
    ...
    const node = document.querySelector("#canvas-wrapper-screenbolt");
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      if (document.querySelector("#screenbolt-zoom-wrap")) return;
      const div = document.createElement("div");
      div.id = "screenbolt-zoom-wrap";
      div.style.width = "100vw";
      div.style.height = "100vh";

      while (
        document.body.firstChild &&
        document.body.firstChild.id !== "screenbolt-ui"
      ) {
        if (document.body.firstChild.id !== "screenbolt-ui") {
      ...
      document.body.appendChild(document.getElementById("screenbolt-ui"));
      zoomSelector.current = document.querySelector("#screenbolt-zoom-wrap");

      observer.current = new MutationObserver((mutations) => {
        if (!contentState.showExtension) {
          mutations.forEach((mutation) => {
            if (mutation.addedNodes.length > 0) {
              const screenboltUi = document.querySelector("#screenbolt-ui");
              if (screenboltUi) {
    ...
    return () => {
      setTimeout(() => {
        ...
        const zoomWrap = document.querySelector("#screenbolt-zoom-wrap");
        if (zoomWrap) {
